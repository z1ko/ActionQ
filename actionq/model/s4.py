...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def _setup_C(self, L):
        ...
        if self.l_kernel.item() == 0:
            ...
            if self.verbose:
                log.info(f"S4: Doubling length from L = {self.l_kernel.item()} to {2*self.l_kernel.item()}")
            double_length = True
    ...
    def _step_state_linear(self, u=None, state=None):
        ...
        if state.size(-1) == self.N:  # Only store half of the conjugate pairs; should be true by default
            ...
            def contract_fn(p, x, y): return contract('r h n, r h m, ... h m -> ... h n', _conj(p), _conj(x), _conj(y))[..., :self.N]  # inner outer product
        else:
            assert state.size(-1) == 2 * self.N
            step_params = {k: _conj(v) for k, v in step_params.items()}
            def contract_fn(p, x, y): return contract('r h n, r h m, ... h m -> ... h n', p, x, y)  # inner outer product
        D = step_params["D"]  # (H N)
        E = step_params["E"]  # (H N)
